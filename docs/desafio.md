...
1. Entrada: O sistema deve receber um CEP válido de 8 dígitos.
2. Identificação de localização: O sistema deve realizar a busca do CEP para encontrar o nome da localização (cidade).
3. Consulta de clima: A partir da localização, o sistema deve consultar a temperatura atual.
4. Conversão: O sistema deve retornar as temperaturas formatadas em Celsius, Fahrenheit e Kelvin.
...
## Entregável
1. Código fonte: Link do repositório no GitHub.
2. URL de acesso: O endereço ativo da aplicação no Google Cloud Run (deve constar no README).
3. Testes: O projeto deve conter testes automatizados.
...
3. README: O arquivo deve conter:
